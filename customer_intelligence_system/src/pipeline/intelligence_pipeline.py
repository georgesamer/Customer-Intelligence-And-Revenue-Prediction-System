...
class CustomerIntelligencePipeline:
    ...
    def create_customer_scores(self):
        ...
        # Calculate Customer Score (normalized HVC_Score + Predicted Revenue percentile)
        # Normalize HVC_Score to 0-100
        ...
        # Priority Flag (HVC + High Risk = Critical)
    ...
    def generate_insights(self):
        ...
        print(f"  • HVC Lifetime Value: {(hvc_revenue/hvc_count) * hvc_data['Frequency'].mean():.0f}")
        ...
        print(f"  • Action Required: IMMEDIATE retention campaigns needed")
        ...
        for i, (segment, row) in enumerate(top_segments.iterrows(), 1):
            ...
            print(f"  {i}. {segment:25s}: ${row['Actual_Revenue']:12,.2f} ({int(row['CustomerID']):4,} customers, {revenue_pct:.1f}%)")
        ...
        print("="*80)
        top_10 = self.results.nlargest(10, 'Customer_Score')[
            ['CustomerID', 'Customer_Score', 'Actual_Revenue', 'Segment', 'Customer_Type', 'Risk_Status', 'Priority_Flag']
        ]
